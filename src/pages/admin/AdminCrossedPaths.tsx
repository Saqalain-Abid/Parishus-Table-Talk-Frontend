...
const generateCrossedPaths = async () => {
    ...
    for (const eventId of eventIds) {
      ...
      for (const attendee of attendees || []) {
        if (attendee.user_id === profile?.id) continue;

        const { data: existing } = await supabase
          .from('crossed_paths')
          .select('id')
          .or(`and(user1_id.eq.${profile?.id},user2_id.eq.${attendee.user_id}),and(user1_id.eq.${attendee.user_id},user2_id.eq.${profile?.id})`)
          .maybeSingle();

        if (!existing) {
          await supabase.from('crossed_paths').insert({
            user1_id: profile?.id,
            user2_id: attendee.user_id,
            matched_at: new Date().toISOString(),
            location_name: event?.location_name || 'Unknown',
            is_active: true
          });
        }
      }
